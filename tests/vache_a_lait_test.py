...
from vaches.exceptions import exception
...
def test_should_start_with_empty_panse_given_new_vache_a_lait():
    ...
    assert vache._panse == 0


def test_should_increase__lait_disponible_given_positive_panse_when_ruminer():
    # Arrange
    vache = VacheALait(petit_nom="Lola", poids=500.0, age=5)
    vache.brouter(10.0)
    ...
    assert vache._lait_disponible == pytest.approx(VacheALait.RENDEMENT_LAIT * 10.0)
...
def test_should_accumulate__lait_disponible_given_two_ruminations():
    # Arrange
    vache = VacheALait(petit_nom="Lola", poids=500.0, age=5)
    ...
    assert vache._lait_disponible == pytest.approx(VacheALait.RENDEMENT_LAIT * (10.0 + 4.0))
...
def test_should_decrease__lait_disponible_given_valid_litres_when_traire():
    # Arrange
    vache = VacheALait(petit_nom="Lola", poids=500.0, age=5)
    vache.brouter(10.0)
    vache.ruminer()
    ...
    # Assert (1 assertion métier)
    assert vache._lait_disponible == pytest.approx((VacheALait.RENDEMENT_LAIT * 10.0) - 3.0)
...
def test_should_raise_invalid_vache_exception_given_litres_greater_than__lait_disponible_when_traire():
    # Arrange
    vache = VacheALait(petit_nom="Lola", poids=500.0, age=5)
    vache.brouter(10.0)
    vache.ruminer()
    ...
    # Act / Assert
    with pytest.raises(exception):
        vache.traire(vache._lait_disponible + 0.0001)
...
def _produire_lait_jusqua(vache: VacheALait, cible_lait: float) -> None:
    ...
    while vache._lait_disponible < cible_lait:
        restant = cible_lait - vache._lait_disponible
        ...
        if vache._lait_disponible > cible_lait + 1e-9:
            break
...
def test_should_allow_ruminer_given_lait_production_reaches_production_max_exactly():
    ...
    # Assert (1 assertion métier)
    assert vache._lait_disponible == pytest.approx(VacheALait.PRODUCTION_LAIT_MAX)
